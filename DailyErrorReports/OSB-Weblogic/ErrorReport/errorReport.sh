...
while read serverDetails; do
   ...
   WL_LOG_DIR=`echo  $serverDetails | cut -d ',' -f 3`
   ssh $server "bash -s" < $SCRIPT_HOME/errorReportOther.sh "$serverInstance" "$WL_LOG_DIR" >>  $LOGS_DIR/log.txt #Get reports from other nodes.
...
then
	echo "TO:$mailingList"  > $LOGS_DIR/mail.txt
        echo "Subject: $ENV - OSB: Error Report" >>  $LOGS_DIR/mail.txt
